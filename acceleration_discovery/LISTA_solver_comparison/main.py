...
def setup_model(config , **kwargs) :
    ...
    if config.net == 'lrn_U_LISTA' :
        """lrn_U_LISTA"""
        config.model = ("lrn_U_LISTA_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.lrn_U_LISTA import lrn_U_LISTA
        model = lrn_U_LISTA (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)
    if config.net == 'lrn_U_LISTA_cpss' :
        """lrn_U_LISTA-CPSS"""
        config.model = ("lrn_U_LISTA_cpss_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.lrn_U_LISTA_cpss import lrn_U_LISTA_cpss
        model = lrn_U_LISTA_cpss (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)

    if config.net == 'lrn_U_LFISTA' :
        """lrn_U_LFISTA"""
        config.model = ("lrn_U_LFISTA_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.lrn_U_LFISTA import lrn_U_LFISTA
        model = lrn_U_LFISTA (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)
    if config.net == 'lrn_U_LFISTA_cpss' :
        """lrn_U_LFISTA-CPSS"""
        config.model = ("lrn_U_LFISTA_cpss_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.lrn_U_LFISTA_cpss import lrn_U_LFISTA_cpss
        model = lrn_U_LFISTA_cpss (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)
        
    if config.net == 'lrn_U_LFISTA_m' :
        """lrn_U_LFISTA_m"""
        config.model = ("lrn_U_LFISTA_m_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.lrn_U_LFISTA_m import lrn_U_LFISTA_m
        model = lrn_U_LFISTA_m (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)
    if config.net == 'lrn_U_LFISTA_cpss_m' :
        """lrn_U_LFISTA-CPSS-momentum search"""
        config.model = ("lrn_U_LFISTA_m_cpss_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.lrn_U_LFISTA_cpss_m import lrn_U_LFISTA_cpss_m
        model = lrn_U_LFISTA_cpss_m (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)

    if config.net == 'mod_A_LFISTA_m' :
        """mod_A_LFISTA_m"""
        config.model = ("mod_A_LFISTA_m_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.mod_A_LFISTA_m import mod_A_LFISTA_m
        model = mod_A_LFISTA_m (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)

    if config.net == 'mod_A_LFISTA_cpss_m' :
        """mod_A_LFISTA-CPSS-momentum search"""
        config.model = ("mod_A_LFISTA_cpss_m_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.mod_A_LFISTA_cpss_m import mod_A_LFISTA_cpss_m
        model = mod_A_LFISTA_cpss_m (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)

    if config.net == 'fix_A_LISTA' :
        """fix_A_LISTA"""
        config.model = ("fix_A_LISTA_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.fix_A_LISTA import fix_A_LISTA
        model = fix_A_LISTA (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)
    if config.net == 'fix_A_LISTA_cpss' :
        """fix_A_LISTA-CPSS"""
        config.model = ("fix_A_LISTA_cpss_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.fix_A_LISTA_cpss import fix_A_LISTA_cpss
        model = fix_A_LISTA_cpss (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)
    if config.net == 'fix_A_LFISTA' :
        """fix_A_LFISTA"""
        config.model = ("fix_A_LFISTA_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.fix_A_LFISTA import fix_A_LFISTA
        model = fix_A_LFISTA (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)
    if config.net == 'fix_A_LFISTA_cpss_s' :
        """fix_A_LFISTA-CPSS_s"""
        config.model = ("fix_A_LFISTA_cpss_s_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.fix_A_LFISTA_cpss_s import fix_A_LFISTA_cpss_s
        model = fix_A_LFISTA_cpss_s (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)

    if config.net == 'LFISTA' :
        """LFISTA"""
        config.model = ("LFISTA_T{T}_lam{lam}_{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.LFISTA import LFISTA
        model = LFISTA (kwargs['A'], T=config.T, lam=config.lam,
                       untied=config.untied, coord=config.coord,
                       scope=config.scope)

    if config.net == 'LFISTA_cpss' :
        """LFISTA-CPSS"""
        config.model = ("LFISTA_cpss_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.LFISTA_cpss import LFISTA_cpss
        model = LFISTA_cpss (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)
    
    if config.net == 'LFISTA_cpss_s' :
        """LFISTA-CPSS-simplified"""
        config.model = ("LFISTA_cpss_s_T{T}_lam{lam}_p{p}_mp{mp}_"
                        "{untiedf}_{coordf}_{exp_id}"
                        .format (T=config.T, lam=config.lam, p=config.percent,
                                 mp=config.max_percent, untiedf=untiedf,
                                 coordf=coordf, exp_id=config.exp_id))
        from models.LFISTA_cpss_s import LFISTA_cpss_s
        model = LFISTA_cpss_s (kwargs['A'], T=config.T, lam=config.lam,
                            percent=config.percent, max_percent=config.max_percent,
                            untied=config.untied , coord=config.coord,
                            scope=config.scope)        

    if config.net == "ALFISTA":
        """ALFISTA"""
        config.model = ("ALFISTA_T{T}_lam{lam}_p{p}_mp{mp}_{W}_{coordf}_{exp_id}"
        ...
        from models.ALFISTA import ALFISTA
        model = ALFISTA(kwargs['A'], T=config.T, lam=config.lam, W=W,
                       percent=config.percent, max_percent=config.max_percent,
...
def run_sc_train_m(config) :
    """Run sparse coding train for momentum search"""
    if not os.path.exists(config.probfn):
        raise ValueError ("Problem file not found.")
    else:
        p = problem.load_problem(config.probfn)
    ...
    y_, x_, y_val_, x_val_ = (
        train.setup_input_sc (
            config.test, p, config.tbs, config.vbs, config.fixval,
            config.supp_prob, config.SNR, config.magdist, **config.distargs))
    # customized for momentum search
    """Set up training.""" 
    stages = train.setup_sc_training_m (
            model, y_, x_, y_val_, x_val_, None,
            config.init_lr, config.decay_rate, config.lr_decay)
...
def run_sc_test (config) :
    ...
    xt = np.load (config.expbase+'/prob.npz')['x']
    ...
    recon_gts = dict(recon=np.asarray(xh), gt=np.asarray(xt))
    np.savez (config.resfn , **res)
    np.savez ('recon_check/recon.npz', **recon_gts)
    # end of test
    

def run_sc_test_m (config) :
    """
    Test model.
    """
